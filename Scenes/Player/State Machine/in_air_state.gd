...
func verifications():
	if character.hit_ground_cooldown != character.hit_ground_cooldown_reference: character.hit_ground_cooldown = character.hit_ground_cooldown_reference
...
func applies(delta : float):
	if !character.is_on_floor(): 
		if character.coyote_jump_cooldown > 0.0: character.coyote_jump_cooldown -= delta

func check_input():
	if Input.is_action_just_pressed("jump"):
		# Checa se pode dar um buffered jump
		if character.floor_check.is_colliding() and character.last_frame_position.y > character.position.y and character.number_air_jump <= 0: character.buffered_jump_on = true
		# Checa se pode fazer um coyote jump
...
func check_if_floor():
	if character.is_on_floor():
		if character.buffered_jump_on: 
			...
			if character.move_direction: transitioned.emit(self, character.walk_or_run)
...
func move(delta : float):
	...
	if !character.is_on_floor():
		if character.move_direction:
			if character.desired_move_speed < character.max_speed: character.desired_move_speed += character.bunny_hop_desired_move_speed_increase * delta
