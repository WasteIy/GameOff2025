...
func sway_weapon(delta, is_idle : bool) -> void:
	...
	if is_idle:
		...
		position.x = lerp(position.x, mesh_container.position.x - (mouse_movement.x * sway_ammount_position + random_sway_x + weapon_bob_amount.x) * delta, sway_speed_position)
		position.y = lerp(position.y, mesh_container.position.y + (mouse_movement.y * sway_ammount_position + random_sway_y + weapon_bob_amount.y) * delta, sway_speed_position)
		rotation_degrees.y = lerp(rotation_degrees.y, mesh_container.rotation_degrees.y + (mouse_movement.y * sway_ammount_rotation) * delta, sway_speed_rotation)
		rotation_degrees.x = lerp(rotation_degrees.y, mesh_container.rotation_degrees.y - (mouse_movement.x * sway_ammount_rotation) * delta, sway_speed_rotation)

func weapon_bob(delta, bob_speed : float, horizontal_bob_amount : float, vertical_bob_amount : float) -> void:
	time += delta
	weapon_bob_amount.x = -sin(time * bob_speed) * horizontal_bob_amount
	weapon_bob_amount.y = abs(cos(time * bob_speed) * vertical_bob_amount) - 2
