...
func update_animation(delta):
	...
	if player.state_machine.current_state.name == "IdleState":
		current_weapon.sway_weapon(delta, true)
	if player.state_machine.current_state.name == "WalkState":
		current_weapon.sway_weapon(delta, false)
		current_weapon.weapon_bob(delta, 3.0, 3.0, 1.0)
	if player.state_machine.current_state.name == "RunState":
		current_weapon.sway_weapon(delta, false)
		current_weapon.weapon_bob(delta, 4.0, 10.0, 3.0)
	if player.state_machine.current_state.name == "CrouchState":
		current_weapon.sway_weapon(delta, false)
		current_weapon.weapon_bob(delta, 1.5, 3.0, 0.5)
